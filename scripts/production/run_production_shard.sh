...
if [[ ${WHIZARD_SKIP_ENV_SETUP:-0} != 1 ]]; then
  ...
  [[ -r "$KEY4HEP_SETUP" ]] || {
    echo "ERROR: cannot read Key4HEP setup: $KEY4HEP_SETUP" >&2
    exit 7
  }

  # Key4HEP's setup scripts access positional parameters such as $1.
  # They are therefore not safe to source while Bash nounset is active.
  # Disable nounset only during environment setup and restore it afterward.
  nounset_was_on=0
  case $- in
    *u*)
      nounset_was_on=1
  ...
  if (( setup_rc != 0 )); then
    echo "ERROR: Key4HEP setup failed with return code $setup_rc: $KEY4HEP_SETUP" >&2
    exit 7
  fi
fi

command -v whizard >/dev/null 2>&1 || {
  echo "ERROR: whizard is not available after environment setup" >&2
  exit 8
}
...
ensure_directory() {
  ...
  # Normally all persistent campaign directories are prepared serially by
  # submit_production.py.  This is a fallback for manually launched jobs and
  # transient EOS namespace visibility.
  if [[ -d "$directory" ]]; then
    return 0
  fi

  for attempt in 1 2 3 4 5; do
    # EOS/FUSE can report EEXIST during concurrent recursive creation even
    # when the directory is being created successfully by another client.
  ...
  echo "ERROR: unable to prepare persistent directory after retries: $directory" >&2
  return 1
}

for directory in   "$CAMPAIGN_ROOT/runs/$SAMPLE_ID"   "$RAW_LHE_DIR"   "$FINAL_LHE_DIR"   "$LOG_DIR"   "$CAMPAIGN_ROOT/failed/$SAMPLE_ID"
do
