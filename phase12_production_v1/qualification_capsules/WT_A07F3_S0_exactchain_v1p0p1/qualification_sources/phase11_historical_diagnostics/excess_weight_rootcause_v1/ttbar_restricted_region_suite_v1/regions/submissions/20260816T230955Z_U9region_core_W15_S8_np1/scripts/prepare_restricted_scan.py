...
def main():
 ...
 for ar in adaptive:
  for sr in seeds:
   ...
   dag += [f'JOB {pn} {parent}',f'VARS {pn} mode="{pm}" adaptive_id="{aid}" seed_id="{sid}"']
   ...
   for fr in fset:
    ...
    dag += [f'JOB {cn} {child}',f'VARS {cn} mode="{cm}" parent_mode="{pm}" adaptive_id="{aid}" fixed_id="{fid}" seed_id="{sid}"']
 ...
 for sr in seeds:
  ...
  dag += [f'JOB {node} {ctl}',f'VARS {node} mode="{mode}" seed_id="{sid}"','']
