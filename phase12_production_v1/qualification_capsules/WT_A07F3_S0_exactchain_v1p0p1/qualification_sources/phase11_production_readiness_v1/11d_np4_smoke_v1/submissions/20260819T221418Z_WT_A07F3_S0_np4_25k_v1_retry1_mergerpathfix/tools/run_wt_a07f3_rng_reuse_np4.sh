...
while (($#)); do
  case "$1" in
    ...
    --grid-seed)
      GRID_SEED=${2:?}; shift 2 ;;
    --pre-integration-seed)
      PRE_INTEGRATION_SEED=${2:?}; shift 2 ;;
    --generation-seed)
...
for V in \
  ...
  PRE_INTEGRATION_SEED GENERATION_SEED EVENTS
do
  [[ -n "${!V}" ]] || {
...
# Do not reseed here.
#
# Phase-11C RNG-controller qualification demonstrated that the
# shard-defining RNG seed must be active before integrate(proc_epmum).
# The RNG state then continues into event generation.
simulate (proc_epmum) {
  n_events = $EVENTS
  sample_format = lhef
  \$sample = "$SAMPLE"
...
echo "GRID_CONSTRUCTION_SEED=$GRID_SEED"
echo "PRE_INTEGRATION_RNG_SEED=$PRE_INTEGRATION_SEED"
echo "POST_SIMULATION_RNG_SEED=$GENERATION_SEED"
echo "EVENTS=$EVENTS"
...
WHIZARD_EXE="$(
  readlink -f "$(command -v whizard)"
)"

MPI_MERGER=/afs/cern.ch/user/c/cglenn/FCCWork/whizard/whizard_ttbar_spinpol/phase11_production_readiness_v1/tools/np4_generation_v1/merge_mpi_lhe.py

if [[ ! -s "$MPI_MERGER" ]]; then
  echo "ERROR: MPI LHE merger missing: $MPI_MERGER" >&2
  exit 2
fi

MPI_MERGE_RC=0
LHE_LAYOUT="whizard_mpi_rank_merge"
LHE_RANK_MANIFEST="$PWD/lhe_rank_manifest.json"

echo "MPI_RANKS=$MPI_RANKS"
echo "OMP_NUM_THREADS=$OMP_NUM_THREADS"
echo "WHIZARD_EXE=$WHIZARD_EXE"
echo "MPI_MERGER=$MPI_MERGER"
echo "LHE_LAYOUT=$LHE_LAYOUT"
...
if (( WHIZARD_RC == 0 && MPI_MERGE_RC != 0 )); then
  WHIZARD_RC=$MPI_MERGE_RC
fi
...

{
  ...
  echo "OMP_NUM_THREADS=$OMP_NUM_THREADS"
  echo "LHE_LAYOUT=$LHE_LAYOUT"
  echo "MPI_MERGE_RC=$MPI_MERGE_RC"
...
if [[ -s "$LHE_RANK_MANIFEST" ]]; then
  cp -a "$LHE_RANK_MANIFEST" "$DEST/"
fi
