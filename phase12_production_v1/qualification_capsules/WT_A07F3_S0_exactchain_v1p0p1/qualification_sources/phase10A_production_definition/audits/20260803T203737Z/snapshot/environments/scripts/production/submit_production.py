...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--submission-root",
        type=Path,
        default=root / "condor/runtime_submissions",
        help=(
            "AFS/local control directory for concrete submit files, itemdata, "
            "Condor stdout/stderr, and the live event log"
        ),
    )
...
def main() -> int:
    ...
    # Prepare the entire persistent EOS namespace serially on the submit
    # node.  Letting many worker jobs recursively create the same EOS parent
    # directories can produce transient EEXIST failures through EOS/FUSE.
    common_directories = [
        root / "runs",
        root / "lhe_raw",
        root / "lhe",
    ...
    for directory in common_directories:
        ...
        if not directory.is_dir():
            raise RuntimeError(
                f"failed to prepare campaign directory: {directory}"
            )

    for cfg in configs:
        for category in ("runs", "lhe_raw", "lhe", "logs", "failed"):
            directory = root / category / cfg.sample_id
            directory.mkdir(parents=True, exist_ok=True)

            if not directory.is_dir():
                raise RuntimeError(
                    f"failed to prepare sample directory: {directory}"
                )
            ...
            jobs.append(
                {
                    ...
                    "job_memory_mb": cfg.request_memory_mb,
    ...
    control_campaign = f"{campaign_id}{'_smoke' if args.smoke else ''}"

    # Live Condor control files belong on AFS/local storage.  Persistent
    # production products and provenance remain in the EOS campaign tree.
    submission_dir = (
        args.submission_root.expanduser().resolve()
        / control_campaign
        / stamp
    )
    provenance_dir = root / "manifests" / "submissions" / stamp

    stdout_root = submission_dir / "stdout"
    stderr_root = submission_dir / "stderr"
    ...
    submission_log = submission_dir / "whizard_production.condor.log"
    ...
    record = {
        ...
        "jobs_file": str(itemdata),
        "submission_control_dir": str(submission_dir),
    ...
    def sync_provenance() -> None:
        """Copy submission metadata into the persistent EOS campaign tree."""
        for name in (
            "jobs.itemdata",
            "jobs.csv",
            "skipped.csv",
    ...
    def queued_cluster_ids() -> set[str]:
    ...
    print(f"Control dir:   {submission_dir}")
    print(f"EOS record:    {provenance_dir}")
    print("Command:")
    print("  " + " ".join(command))
    ...
    before_clusters = queued_cluster_ids()

    try:
        # Inherit stdout/stderr so authentication, schedd selection, warnings,
        # and submission progress remain visible to the user.
        result = subprocess.run(command, text=True)
    except KeyboardInterrupt:
        print(
            "Submission interrupted. Check condor_q for this batch name "
            "before trying again; the schedd may already have accepted it.",
            file=sys.stderr,
        )
        return 130
    ...
    after_clusters = queued_cluster_ids()
    new_clusters = sorted(
        after_clusters - before_clusters,
        key=int,
    )

    if not new_clusters and after_clusters:
        new_clusters = [max(after_clusters, key=int)]

    record["submitted"] = True
    record["submitted_utc"] = utc_now()
    record["condor_cluster_id"] = ",".join(new_clusters)
    ...
    print(f"Updated {provenance_dir / 'submission.json'}")
    return 0
