...
def main() -> None:
    ...
    (a.outdir / "C5GA_RESULT.json").write_text(json.dumps(result, indent=2, sort_keys=True) + "\n")
    with (a.outdir / "C5GA_RESULT.txt").open("w") as f:
        f.write("Phase 11C.5G-A generation-envelope stress result\n")
        f.write("================================================\n\n")
        f.write(f"C5GA_STATUS={status}\n")
        f.write(f"C5GA_TOTAL_EVENTS={total_events_all}\n")
        if a.audit_lhe:
            f.write(f"C5GA_GLOBAL_UNIQUE_EVENTS={result['global_unique_events']}\n")
            f.write(f"C5GA_DUPLICATE_EVENT_HASHES={result['duplicate_event_hashes']}\n")
    ...
    output_files = [shard_path, grid_path, issues_path, a.outdir / "C5GA_RESULT.json", a.outdir / "C5GA_RESULT.txt", sha_path]
    with out_sha.open("w") as f:
        for p in output_files:
            f.write(f"{file_sha256(p)}  {p}\n")

    print(f"C5GA_STATUS={status}")
    for g in grid_rows:
        print(
            f"{g['grid']}: gate={'PASS' if g['grid_gate'] else 'FAIL'} "
            f"technical={'PASS' if g['technical_pass'] else 'FAIL'} "
    ...
    if a.audit_lhe:
        print(f"C5GA_GLOBAL_UNIQUE_EVENTS={result['global_unique_events']}")
        print(f"C5GA_DUPLICATE_EVENT_HASHES={result['duplicate_event_hashes']}")
    print(f"C5GA_NEXT_ACTION={next_action}")
    raise SystemExit(rc)
