...
EXPECTED_GRIDS = ("S3", "S4", "S5", "S6", "S7")
EXPECTED_SHARDS_PER_GRID = 20
EXPECTED_EVENTS_PER_SHARD = 500
EXPECTED_EVENTS_PER_GRID = 10_000
EXPECTED_TOTAL_EVENTS = len(EXPECTED_GRIDS) * EXPECTED_EVENTS_PER_GRID
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Collect/freeze Phase 11C S3-S7 generation-envelope qualification results.")
...
def main() -> None:
    ...
    if (
        not all_technical
        or total_events_all != EXPECTED_TOTAL_EVENTS
        or structural_issues
    ):
        status = "INCOMPLETE_TECHNICAL"
        rc = 3
        next_action = (
            "Rerun only genuine technical-failure modes with the SAME "
            "mode/grid/seed; do not substitute RNG seeds."
        )
        ...
        rc = 0
        next_action = (
            "S3-S7 generation-envelope qualification passed. "
            "Combine with the already-qualified S0-S2 result, freeze the "
            "final Phase-11C physics-analysis contract, and build the fresh "
            "SINGLE-versus-MULTI holdout."
        )

    else:
        status = "FAIL_GRID_GATES"
        rc = 2
        next_action = (
            "At least one S3-S7 grid failed a frozen generation-envelope "
            "gate. Do not substitute another grid or RNG stream. Inspect the "
            "failing grid and revisit the qualified prescription before the "
            "final physics holdout."
        )
    ...
    with (a.outdir / "WT11C_RESULT.txt").open("w") as f:
        f.write("Phase 11C S3-S7 generation-envelope qualification result\n")
        f.write("================================================\n\n")
