...
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module opr_t_decay_i1_computation
  use kinds
  use omega95
  ...
  data table_flavor_states(:,   1) /   6,   5, -11,  12 / ! t b e+ nue

  integer, dimension(n_cindex,n_prt,n_cflow), save, protected :: table_color_flows
  data table_color_flows(:,:,   1) / 1,0,  1,0,  0,0,  0,0 /
  ...
  type thread_local_data
    type(spinor) :: owf_fu3_i1_p1, owf_fl1_p3
    type(conjspinor) :: owf_fd3b_o1_p2, owf_fn1b_p4
    type(vector) :: owf_fwp_p12
    complex(kind=default) :: oks_fu3_i1_fd3_i1_fl1b_fn1
...
contains
  ...
  pure function md5sum ()
    ...
    md5sum = "6497DE63A45797FA8C6BF24EBAA9C66C"
  end function md5sum
...
!$OMP PARALLEL DO DEFAULT(SHARED) PRIVATE(s, h, tld) SCHEDULE(STATIC)
    do hi = 1, hel_finite
      ...
      tld%owf_fu3_i1_p1 = u (mass(6), - p1, s(1))
      tld%owf_fd3b_o1_p2 = ubar (mass(5), p2, s(2))
      ...
      amp(1,1,1,h) = tld%oks_fu3_i1_fd3_i1_fl1b_fn1
    end do
!$OMP END PARALLEL DO
  ...
  type(thread_local_data), intent(inout) :: tld
      tld%owf_fwp_p12 = &
         ...
         + vl_ff(gcc,tld%owf_fd3b_o1_p2,tld%owf_fu3_i1_p1))
  end subroutine compute_fusions_0001
  subroutine compute_brakets_0001 (tld)
  type(thread_local_data), intent(inout) :: tld
      tld%oks_fu3_i1_fd3_i1_fl1b_fn1 = 0
      tld%oks_fu3_i1_fd3_i1_fl1b_fn1 = tld%oks_fu3_i1_fd3_i1_fl1b_fn1 &
         + tld%owf_fwp_p12*( + vl_ff(gcc,tld%owf_fn1b_p4,tld%owf_fl1_p3))
      tld%oks_fu3_i1_fd3_i1_fl1b_fn1 = &
         - tld%oks_fu3_i1_fd3_i1_fl1b_fn1 ! 2 vertices, 1 propagators
      ! unit symmetry factor
  end subroutine compute_brakets_0001

end module opr_t_decay_i1_computation
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! O'Mega API Version 1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module opr_t_decay_i1_api_v1
  use opr_t_decay_i1_computation, only: number_particles_in, &
  ...
  implicit none
  public
end module opr_t_decay_i1_api_v1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! Backward compatible alias for O'Mega API Version 1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module opr_t_decay_i1
  use opr_t_decay_i1_api_v1
  implicit none
  public
end module opr_t_decay_i1
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
! O'Mega API Version 3
! WORK IN PROCESS !!!!
! NOT FOR PRODUCTION !
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
module opr_t_decay_i1_api_v3
  use omega_birdtracks
  use omega_api_v3
  use opr_t_decay_i1_computation
  implicit none
  private
...
end module opr_t_decay_i1_api_v3
