...
def _paired_lhe_hepmc_rows(
    ...
    """Measure paired post-shower analyzer migration.

    The event identities and nominal event weights must agree exactly between
    the matched LHE and HepMC samples. The analyzer four-vectors are not
    required to agree because the HepMC charged leptons are primary-W daughters
    after PYTHIA final-state shower recoil and possible QED final-state
    radiation, whereas the LHE charged leptons are hard-process decay products.
    """

    if (
        lhe_sample.event_keys is None
        or hepmc_sample.event_keys is None
    ):
        raise RuntimeError(
            "Paired LHE/HepMC comparison requires event keys "
            "in both ntuples"
        )

    if (
        len(np.unique(lhe_sample.event_keys))
        != len(lhe_sample.event_keys)
    ):
        raise RuntimeError(
            "Duplicate LHE event keys prevent one-to-one pairing"
        )

    if (
        len(np.unique(hepmc_sample.event_keys))
        != len(hepmc_sample.event_keys)
    ):
        raise RuntimeError(
            "Duplicate HepMC event keys prevent one-to-one pairing"
        )

    lhe_index = {
        key: index
        for index, key
        in enumerate(lhe_sample.event_keys)
    }

    hepmc_index = {
        key: index
        for index, key
        in enumerate(hepmc_sample.event_keys)
    }

    common = sorted(
        set(lhe_index) & set(hepmc_index),
        key=str,
    )

    if not common:
        raise RuntimeError(
            "No common event keys for paired LHE/HepMC comparison"
        )

    lhe_indices = np.asarray(
        [
            lhe_index[key]
            for key in common
        ],
    ...
    if not np.allclose(
        ...
        raise RuntimeError(
            "Paired LHE/HepMC event weights differ"
        )
    ...
    comparison_semantics = {
        "comparison_role":
            "postshower_analyzer_migration",

        "first_stage_object":
            "hepmc_primary_W_daughter_after_pythia_recoil",

        "second_stage_object":
            "lhe_hard_primary_top_decay_lepton",

        "strict_analyzer_identity_eligible":
            False,
